...
export default async function handler(req,res){
  ...
  try{
    ...
    await sql`UPDATE mpesa_transactions SET status=CASE WHEN status='success' OR result_message LIKE 'Payment amount mismatch%' OR result_message LIKE 'Payment callback did not include a valid amount%' THEN status ELSE ${status} END,result_code=${String(code)},result_message=${String(r.data.ResultDesc||r.data.ResponseDescription||'')},updated_at=NOW(),completed_at=NOW() WHERE checkout_request_id=${checkoutRequestID}`;
